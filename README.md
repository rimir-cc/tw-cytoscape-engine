...
- **Auto-fit to view** — frames all nodes on load and when the container resizes, and re-frames when the visible node set changes; dragging a node leaves the viewport untouched. The canvas stays hidden until its first framed fit, so it never flashes un-framed on mount (matters when the container's height resolves a frame after init, e.g. a side-preview pane)
